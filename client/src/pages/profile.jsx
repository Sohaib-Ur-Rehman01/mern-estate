...
export default function Profile() {
  ...
  // request.resource.content-Type.matches('image/.*)
  useEffect(() => {
    if (file) {
        ...
        <p className="text-sm self-center">
          {fileUploadError ? (
            <span className="text-red-700">
              Error Image Upload (Image less then 2 mb)
            </span>
      ...
      {userListings && userListings.length > 0 && (
        <div className="flex flex-col gap-3">
          ...
          {userListings.map((listing) => (
            <div
              key={listing._id}
              className="border rounded-lg p-3 flex justify-between items-center font-semibold flex-1  truncate"
            >
              ...
              <Link to={`/listing/${listing._id}`}>
                <p className="text-slate-700 font-semibold hover:underline flex-1">
                  {listing.name}
                </p>
              </Link>
